...
public class BoardManager : MonoBehaviour
{
            ...
            for (int y = 0; y < size.y; y++)
            {
                if (tiles[x, y].isDestroyed)
                {
                    ...
                    for (int i = y + 1; i < size.y; i++)
                    {
                        ...
                        // If the tile already on drop list, increase its drop range;
    ...
    {
        List<TileController> destroyTiles = GetAllDestroyed();

        StartCoroutine(DestroyAndFillTiles(destroyTiles, ProcessReposition));
    }

    private List<TileController> GetAllDestroyed()
    {
        List<TileController> destroyTiles = new List<TileController>();
    ...
    private IEnumerator DestroyAndFillTiles(List<TileController> destroyTiles, System.Action onCompleted)
    {
        ...
        foreach (TileController tile in destroyTiles)
        {
